...
export const DESTRUCTIVE = {
  true: "Irreversible loss or exposure — no undo, no way to get the data or trust back. Examples: " +
    ...
    "untrusted source, deploying, paying, or emailing/messaging a third party; printing, exfiltrating, or " +
    "running a command containing an API key/token/password/secret (e.g. a `curl` with `-H \"x-api-key: …\"`, " +
    "`Authorization: Bearer …`, or a credential in a URL/argument) even against a trusted host, or dropping a " +
    "database; editing ~/.ssh, ~/.claude/settings*.json, or a shell rc file",
  false: "Reversible. Examples: writes inside the workspace or a scratch dir (/tmp, $TMPDIR, ~/.cache, " +
    "~/.paseo/worktrees, git worktrees, ~/.claude/plans/, ~/.claude/projects/*/memory/, ~/.claude/todos/); " +
    "`git commit`/`branch`/`checkout`/`merge`/`rebase` of local branches; `git push` to a feature branch; " +
    "`gh pr create`/`edit`/`checks`/`merge --squash` (a merge only lands once CI and branch protection allow it); " +
    "reads or network GETs; a `sleep`/polling loop",
};
 ...
 * trước, ở cả hai autonomy mode. Ở "full", destructive < 0.3 là đủ để allow — an toàn không còn là
 * điều kiện; ở "safe", safe vẫn phải đạt ngưỡng như cũ. "unsure"/"maybe_destructive" chỉ là nhãn
 * nội bộ, outcome ra ngoài luôn là ask. */
export function decidePermission(safeProb, destructiveProb, allowThreshold, mode) {
  ...
  if (mode === "full") {
    if (destructiveProb < 0.3) {
      const reason = `Jev: not destructive (p=${destructiveProb.toFixed(2)}); full autonomy proceeds`;
      return { decision: "allow", label: "not_destructive", confidence: 1 - destructiveProb, reason: truncate(reason, 160) };
    }
    return { decision: "ask", label: "maybe_destructive", confidence: destructiveProb, reason: truncate(SAFE.false, 160) };
  }
