...
// comes back in ~1.2s, so one retry left most of an 8s budget unused. Retry 5xx (never our own
...
export async function askJev(key, state, questions, source = "cli", budgetMs = TIMEOUT_MS, stateSizes) {
  ...
  try {
    ...
    for (let backoff = BACKOFF_MS; ; backoff *= 2) {
      attempts++;
      try {
        raw = await attempt(key, state, sent, Math.min(perAttempt, deadline - Date.now()));
        break;
      } catch (err) {
        if (!(err.status >= 500 && err.status < 600) || deadline - Date.now() < backoff + MIN_ATTEMPT_MS) throw err;
    ...
    logEvent({
      kind: "call", source, status, error, retried: attempts > 1, attempts, latency_ms: Date.now() - start, model: MODEL,
