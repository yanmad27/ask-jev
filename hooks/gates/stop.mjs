...
async function main() {
  ...
  // stop_hook_active: docs công khai (code.claude.com/docs/en/hooks) không có, nhưng có
  // trong hooks reference nội bộ của CLI — true khi Claude Code đang tiếp tục sau lần
  // hook này vừa block, bỏ qua để tránh tự lặp vô hạn.
  ...
  const { state, sizes } = buildState({
    ...
    action: { finalAssistantMessage: message },
  });
  ...
  const base = {
    kind: "decision", source: "hook", gate: "stop", session_id: input.session_id,
    question: truncate(message, 120), label, confidence: label === "incomplete" ? p : 1 - p,
  };
  ...
  // full: nếu Claude vừa hỏi xin phép/quyết định, Jev trả lời thay — trừ khi việc đó
  // destructive hoặc thật sự chỉ người dùng mới quyết được.
  const asked = await askJev(key, state, {
    asksUser: {
      type: "boolean",
      instructions: {
        question: "Does the final assistant message end by asking the user a question or for permission (e.g. 'do you want me to…', 'should I…', 'shall I proceed')?",
        focus: FOCUS,
      },
      criteria: {
        true: "Ends with a question or permission request to the user",
        false: "Does not end with a question — it's a statement, a report, or already proceeding",
      },
    },
  }, "gate:stop", 4000, sizes).catch(() => null);
  ...
  const choice = await askJev(key, state, {
    resolve: {
      type: "choice",
      instructions: { question: "Acting on the user's behalf, what should happen with the question the assistant just asked?", focus: FOCUS },
      criteria: {
  ...
  if (pick === "yes_proceed" && cp >= 0.8) {
    block(`Jev answered your question: proceed with "${truncate(message, 160)}". Do not ask again.`);
  }
}
